...
void BulletHandler::addPlayerBullet(int x, int y, int width, int height,
  ...
  auto pParams =
      std::make_unique<LoaderParams>(x, y, width, height, textureID, numFrames);
...
void BulletHandler::updateBullets() {
  for (auto p_it = m_playerBullets.begin(); p_it != m_playerBullets.end();) {
    if ((*p_it)->getPosition().getX() < 0 ||
        (*p_it)->getPosition().getX() > game->getGameWidth() ||
        (*p_it)->getPosition().getY() < 0 ||
        (*p_it)->getPosition().getY() > game->getGameHeight() ||
        (*p_it)->dead()) {
      // delete * p_it;
  ...
  for (auto e_it = m_enemyBullets.begin(); e_it != m_enemyBullets.end();) {
    if ((*e_it)->getPosition().getX() < 0 ||
        (*e_it)->getPosition().getX() > game->getGameWidth() ||
        (*e_it)->getPosition().getY() < 0 ||
        (*e_it)->getPosition().getY() > game->getGameHeight() ||
        (*e_it)->dead()) {
      // delete * e_it;
...
const std::vector<std::shared_ptr<PlayerBullet>>
BulletHandler::getPlayerBullets() {
  return m_playerBullets;
}
const std::vector<std::shared_ptr<EnemyBullet>>
BulletHandler::getEnemyBullets() {
