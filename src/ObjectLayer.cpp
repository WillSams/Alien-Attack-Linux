...
void ObjectLayer::update(Level *pLevel) {
  ...
  m_collisionManager.checkEnemyPlayerBulletCollision(
  ...
  if (pLevel->getPlayer()->getPosition().getX() +
          pLevel->getPlayer()->getWidth() <
      game->getGameWidth()) {
  ...
  if (!m_gameObjects.empty()) {
    ...
    {
      if ((*it)->getPosition().getX() <= game->getGameWidth()) {
        (*it)->setUpdating(true);
        (*it)->update();
      } else {
        if ((*it)->type() != std::string("Player")) {
          (*it)->setUpdating(false);
          (*it)->scroll(game->getScrollSpeed());
        } else {
          (*it)->update();
        }
      }
      ...
      if ((*it)->getPosition().getX() < (0 - (*it)->getWidth()) ||
          (*it)->getPosition().getY() > (game->getGameHeight()) ||
          ((*it)->dead())) {
        delete *it;
...
void ObjectLayer::render() {
  for (unsigned int i = 0; i < m_gameObjects.size(); i++) {
    if (m_gameObjects[i]->getPosition().getX() <= game->getGameWidth()) {
