...
bool StateParser::parseState(const char *stateFile, string stateID,
  ...
  for (auto e = pStateRoot->FirstChildElement(); e != nullptr;
       e = e->NextSiblingElement()) {
...
void StateParser::parseTextures(TiXmlElement *pStateRoot,
                                std::vector<std::string> *pTextureIDs) {
  // for each texture we get the filename and the desired ID and add them to the
  // texture manager
  for (auto e = pStateRoot->FirstChildElement(); e != nullptr;
       e = e->NextSiblingElement()) {
    ...
    textureManager->load(filenameAttribute, idAttribute, game->getRenderer());
  }
}
...
void StateParser::parseObjects(TiXmlElement *pStateRoot,
  ...
  for (auto e = pStateRoot->FirstChildElement(); e != nullptr;
    ...
    // numFrames, void()
    GameObject *pGameObject = gameObjFactory->create(e->Attribute("type"));
