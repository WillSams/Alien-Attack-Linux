...
Level *LevelParser::parseLevel(const char *levelFile) {
  ...
  std::cout << "Tile Width:" << pRoot->Attribute("tilewidth")
            << " - Tile Height:" << pRoot->Attribute("tileheight") << "\n";
  ...
  // we must parse the textures needed for this level, which have been added to
  // properties
  for (auto e = pProperties->FirstChildElement(); e != nullptr;
  ...
  for (auto e = pRoot->FirstChildElement(); e != nullptr;
       e = e->NextSiblingElement()) {
...
void LevelParser::parseTextures(TiXmlElement *pTextureRoot) {
  ...
  textureManager->load(pTextureRoot->Attribute("value"),
                       pTextureRoot->Attribute("name"), game->getRenderer());
}
...
void LevelParser::parseTilesets(TiXmlElement *pTilesetRoot,
  ...
  std::cout << "adding texture "
            ...
            << " with ID " << pTilesetRoot->Attribute("name") << std::endl;
  textureManager->load(
      assetsTag.append(pTilesetRoot->FirstChildElement()->Attribute("source")),
      pTilesetRoot->Attribute("name"), game->getRenderer());
...
void LevelParser::parseObjectLayer(TiXmlElement *pObjectElement,
  ...
  for (auto e = pObjectElement->FirstChildElement(); e != nullptr;
       e = e->NextSiblingElement()) {
    if (e->Value() == std::string("object")) {
      ...
      GameObject *pGameObject = gameObjFactory->create(type);

      // get the property values
      for (auto properties = e->FirstChildElement(); properties != nullptr;
           properties = properties->NextSiblingElement()) {
        if (properties->Value() == std::string("properties")) {
          for (auto property = properties->FirstChildElement();
      ...
      // int x, int y, int width, int height, std::string textureID, int
      // numFrames, void()
...
void LevelParser::parseTileLayer(TiXmlElement *pTileElement,
  ...
  for (auto e = pTileElement->FirstChildElement(); e != nullptr;
       e = e->NextSiblingElement()) {
    if (e->Value() == std::string("properties")) {
      for (auto property = e->FirstChildElement(); property != nullptr;
  ...
  for (TiXmlNode *e = pDataNode->FirstChild(); e != nullptr;
       e = e->NextSibling()) {
  ...
  uncompress((Bytef *)&ids[0], &sizeofids, (const Bytef *)decodedIDs.c_str(),
             decodedIDs.size());
