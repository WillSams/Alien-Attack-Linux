...
TileLayer::TileLayer(int tileSize, const std::vector<Tileset> &tilesets)
    : m_tileSize(tileSize), m_tilesets(tilesets), m_position(0, 0),
      m_velocity(0, 0) {
  m_numColumns = (game->getGameWidth() / m_tileSize) + 1;
  m_numRows = (game->getGameHeight() / m_tileSize);
}

void TileLayer::update(Level *pLevel) {
  if (m_position.getX() <
      ((m_mapWidth * m_tileSize) - game->getGameWidth()) - m_tileSize) {
    m_velocity.setX(game->getScrollSpeed());
  ...
  for (signed int i = 0; i < m_numRows; i++) {
    for (int j = 0; j < m_numColumns; j++) {
      ...
      textureManager->drawTile(
          tileset.name, tileset.margin, tileset.spacing, (j * m_tileSize) - x2,
