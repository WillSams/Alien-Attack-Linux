...
void ScrollingBackground::draw() {
  ...
  SDL_RenderCopyEx(game->getRenderer(),
                   textureManager->getTextureMap()[m_textureID], &m_srcRect1,
                   &m_destRect1, 0, 0, SDL_FLIP_NONE);

  // draw second rect
  SDL_RenderCopyEx(game->getRenderer(),
                   textureManager->getTextureMap()[m_textureID], &m_srcRect2,
                   &m_destRect2, 0, 0, SDL_FLIP_NONE);
}
...
void ScrollingBackground::update() {
  if (count == maxcount) {
    ...
    if (m_destRect2.w >= m_width) {
      ...
      m_srcRect1.w = m_destRect1.w = m_srcRect2Width = m_destRect1Width =
          m_width;
      m_srcRect1.h = m_destRect1.h = m_height;
...
void ScrollingBackground::clean() { ShooterObject::clean(); }
