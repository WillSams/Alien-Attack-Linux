  ...
  if (SDL_NumJoysticks() > 0) {
    for (int i = 0; i < SDL_NumJoysticks(); i++) {
      ...
      if (SDL_JoystickOpen(i)) {
        ...
        // if there was an error initialising a joystick we want to know about
        // it
...
Vector2D *InputHandler::getMousePosition() const { return m_mousePosition; }
...
void InputHandler::onKeyDown() { m_keystates = SDL_GetKeyboardState(0); }

void InputHandler::onKeyUp() { m_keystates = SDL_GetKeyboardState(0); }
